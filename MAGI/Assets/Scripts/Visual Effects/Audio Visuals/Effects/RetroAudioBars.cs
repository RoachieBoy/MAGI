...
namespace Visual_Effects.Audio_Visuals.Effects
{
    public class RetroAudioBars : MonoBehaviour
    {
        ...
        [Range(1f, 20f)]
        [SerializeField] private int numberOfBars = 10;
        ...
        private void FixedUpdate()
        {
            for (var i = 0; i < _gameObjects.Count; i++)
            {
                ...
                _gameObjects[i].transform.position = new Vector3(_gameObjects[i].transform.position.x, scaleY / 2.0f, _gameObjects[i].transform.position.z);
            
                // Check if bars should switch between growing and shrinking
                if (Mathf.Approximately(_gameObjects[i].transform.localScale.y, targetScaleY))
                {
                    _growing = !_growing;
                }
            }
